...
public:
  ...
  void begin(uint8_t leftdev, uint8_t rightdev, MD_MAX72XX *M, uint16_t maxDelay);
  void reinit();
  void animate(void);
...
protected:
  ...
  // display parameters
  MD_MAX72XX  *_M;
  uint8_t   _ld;  // left device
  uint8_t   _rd;  // right device

  // blinking parameters
  uint32_t  _lastBlinkTime;
  uint16_t  _currentDelay;
  uint8_t   _blinkState;
  uint8_t   _savedLeftEyeball[EYEBALL_ROWS];
  uint8_t   _savedRightEyeball[EYEBALL_ROWS];
  uint8_t   _blinkLine;
